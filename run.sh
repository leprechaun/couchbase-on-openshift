...
worker(){
	...
	set -x
	couchbase-cli rebalance --cluster=couchbase-os-inter-node --user=$USERNAME --password=$PASSWORD --server-add=${IP} --server-add-username=$USERNAME --server-add-password=$PASSWORD
